...
> 🛑 **墙体部分已退役 — 勿作实施依据**（2026-07-27）
> 背景层现行正本 = `2026-07-27-void-field-v1-deep-space-design.md`（砖墙整条线已退役）：本文的「翻板砖墙 ~80° +
> 邻域砖池 + 光从墙里照出来」已被 v6 的真砖重力井取代，其残余光源又被 v8 整体删除
> （墙后零光源，禁止复活）。
> **仍然有效**：① 无底纹排版（section 不得持有底色，可读性押字体对比度）——CLAUDE.md
> §2 现行条款即源于此；② 嵌墙按钮语义（Socketed Brick）——现行正本 = CLAUDE.md §7，
> 但 v4.1 遗留的呼吸/backglow/`--btn-dy` 已删。
> 本文其余部分仅存架构决策史。
