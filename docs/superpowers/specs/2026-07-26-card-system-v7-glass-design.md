...
> ⚠️ 本文中一切「v6 灯箱墙 / 墙后指针灯 / 砖缝光」的**背景侧叙述已两次作废**：
> 背景层现行正本 = `2026-07-27-void-field-v1-deep-space-design.md`（深空引力场，
> 砖墙整条线 v6→v8→v9 已全部退役，背景层零光源）。
> **卡片侧的材质结论不受影响**，只是锚点换了：毛玻璃现在揉的是星云带的弥漫光，
> 内反射 **135deg 朝左上恒定不变**（v6 对着余晖、v8 对着掠射光、现在对着星云带
> 亮区——三代同向，见 void-field spec §4.3；朝右上的 225deg 始终禁止回改）。
