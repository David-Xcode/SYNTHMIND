...
- **布局连锁**：Archivo 14px + 0.14em 字距下 SYNTHMIND ≈ 105–115px 宽
  （x14 起 → 尾 ≈ x125），左栏图形比例尺需右移/缩短（如 x140..188 三段），
...
- **关键帧**（invisible-hold 技巧，无需 opacity gate）：
  ...
  `12% { stroke-dashoffset: -100 }`（dash 已滑出终点，不可见）→
  `100% { -100 }`（hold，零重绘）。一次可见爆发 ≈ 周期 ×12%。
- **节律**：周期 9/11/13/15 互质 + delay 交错（4.2/6.8/9.4/12s 一类），
  与 seamPulse（7s 系）/modDrift 相位岔开；**任意时刻 ≤1 张面在重栅格化**
  是编排验收线。方向 = 内容流向自上而下（文档→核心→焊盘）；seam 光是
  自下而上的「呼吸」，两者叙事正交不冲突。**不做跨段精确接力**——模块漂移
  本就会拉开缝口，接力假象无法维持，各段独立节律即为定案。
...
2. **§6 白名单 +3 条**（连参数上限一并入册）：
   - ✅ `trace-pulse` — 走线数据脉冲（stroke-dashoffset invisible-hold 间歇式；
     dash 4/96 @ pathLength 100、头 α≤0.85、零 blur；周期 ≥9s 互质错峰、
