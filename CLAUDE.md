...
### Layout Rhythm
Alternate `align="left"` and `align="right"` between consecutive sections for **asymmetric rhythm**. Do NOT center-align every section — the alternating alignment is a core Neural pattern.

---

## 9. Figma MCP Integration Flow (Future Reference)

When a Figma MCP server is connected, follow this workflow:

1. `get_design_context` — understand the design structure
2. `get_screenshot` — visual reference for the target component
3. Download assets if needed → store in `public/`
4. **Translate** Figma output to Neural conventions:
   - Map Figma colors → Neural Tailwind tokens (accent, bg-*, txt-*)
   - Map Figma fonts → Sora/Manrope/JetBrains Mono (display/sans/mono)
   - Map Figma cards → GlassCard component variants
   - Map Figma animations → AnimateOnScroll only
5. **Reuse** existing Neural components before creating new ones
6. Validate against rules in this document

---

## 10. Asset Handling

- If Figma MCP returns `localhost` image sources, use them directly
- Downloaded assets go in `public/`
- **DO NOT** install icon packages (no `lucide-react`, no `heroicons`, no `react-icons`)
- Use inline SVG for navigation controls (chevrons, arrows), CSS shapes for decorative elements
- Prefer typography (numbers, labels) and animation over icons for content sections
- Optimize images before committing (prefer WebP/AVIF)

---

## 11. Technical Constraints

- **tsconfig target: `es5`** — use `.forEach()` instead of `for...of` on Map/Set iterators
...
- **Direct `@google/generative-ai` SDK** for chat — NOT Vercel AI SDK
- **Email via Resend** — `RESEND_API_KEY` in `.env`

---

## 12. Quick Reference — Common Mistakes to Avoid

| Mistake | Correct Approach |
|---------|-----------------|
| Using `backdrop-blur` on elevated cards | Only `.card-surface` uses blur; elevated uses solid bg |
| Hardcoding `#4A9FE5` | Use `text-accent` or `bg-accent` token |
| Using `font-serif italic` | Use `font-display font-semibold` instead |
| Adding shimmer/float/noise/parallax CSS | Only `reveal` animation allowed |
| Creating card CSS from scratch | Use `<GlassCard variant="...">` |
| Center-aligning all SectionTitles | Alternate `left` ↔ `right` for rhythm |
| Using `font-mono` for code blocks | `font-mono` is for eyebrow labels & stats |
| Installing icon packages | Use inline SVG |
| Using `for...of` on iterators | Use `.forEach()` (ES5 target) |
| Using old brass colors `rgba(200,169,126,...)` | Use blue `rgba(74,159,229,...)` with slightly higher opacity |
